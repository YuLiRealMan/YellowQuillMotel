...
CREATE TABLE room (
    ...
    bed_count INTEGER NOT NULL, -- Number of beds (e.g., 1 for single, 2 for double/queen beds)
    room_type VARCHAR(50) NOT NULL,
    base_rate DECIMAL(6, 2) NOT NULL,
    is_pet_friendly BOOLEAN DEFAULT FALSE,
    is_smoking_friendly BOOLEAN DEFAULT FALSE,
    is_available BOOLEAN DEFAULT TRUE,
    CONSTRAINT valid_room_number CHECK (room_number BETWEEN 1 AND 20),
    CONSTRAINT valid_bed_count CHECK (bed_count BETWEEN 1 AND 2)
);

-- Create the Booking table (renamed from "order" to avoid reserved keyword)
